...
void GPIO_ExternalLEDSPASSENGER (void){

    SYSCTL_RCGCGPIO_REG |= 0x02;    /* Enable clock for PORTB and wait for clock to start */
    while(!(SYSCTL_PRGPIO_REG & 0x02));
    //(UNLOCKING AND ENABLING CHANGES ARE NOT NEEDED )
    GPIO_PORTB_AMSEL_REG &= 0xF8;                        /* Disable Analog on PB 0~2 */
    GPIO_PORTB_AFSEL_REG &= 0xF8;                        /* Disable alternative function on PB 0~2 */
    GPIO_PORTB_PCTL_REG  &= 0xFFFFF000;                  /* Clear PMCx bits for PB 0~2 to use as GPIO pins */
    GPIO_PORTB_DIR_REG   |= 0x0E;                        /* Configure PB 0~2 as output pins */
    GPIO_PORTB_DEN_REG   |= 0x0E;                        /* Enable Digital I/O on PB 0~2 */
}


void GPIO_ExtraExternalButtonTriggeredInterruptInit(void){
    // 2nd button for driver to input desired temperature
    GPIO_PORTB_IS_REG    &= ~(1<<3);      /* PB3 detect edges */
    GPIO_PORTB_IBE_REG   &= ~(1<<3);      /* PB3 will detect a certain edge */
    GPIO_PORTB_IEV_REG   &= ~(1<<3);      /* PB3 will detect a falling edge */
    GPIO_PORTB_ICR_REG   |=  (1<<3);       /* Clear Trigger flag for PB3 (Interrupt Flag) */
    GPIO_PORTB_IM_REG    |=  (1<<3);       /* Enable Interrupt on PB3 pin */
    NVIC_PRI0_REG        |=  (5<<13);       /* Set GPIO PORTB priority as 5  */
    NVIC_EN0_REG         |=  (1<<1);      /* Enable NVIC Interrupt for GPIO PORTB by set bit number 1 in EN0 Register */
}
